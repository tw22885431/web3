...
__export(src_exports, {
  ...
  createRollupFilter: () => import_pluginutils2.createFilter,
...
function detectVueVersion(root = import_node_process.default.cwd(), defaultVersion = 3) {
  let isFile = false;
  try {
    isFile = (0, import_node_fs.statSync)(root).isFile();
  } catch {
  }
  const paths = [root];
  if (!isFile) paths.unshift(import_node_path.default.resolve(root, "_index.js"));
  const vuePkg = (0, import_local_pkg.getPackageInfoSync)("vue", { paths });
